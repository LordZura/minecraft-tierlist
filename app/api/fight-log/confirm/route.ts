...
export async function POST(req: NextRequest) {
  ...
  const { data: log, error: fetchError } = await supabase.from("fight_logs").select("*").eq("id", fight_log_id).single();
  ...
  const { error: updateError } = await supabase.from("fight_logs").update({ is_confirmed: true, confirmed_at: new Date().toISOString() }).eq("id", fight_log_id);
