...
export async function POST(req: NextRequest) {
  ...
  if (winner_id !== challenge.challenger && winner_id !== challenge.challenged) return NextResponse.json({ error: "Winner must be a participant." }, { status: 400 });
  ...
  await supabase.from("challenges").update({ [updateField]: newCount }).eq("id", challenge_id);
