...
type FightLog = {
  ...
  player2: string;
  winner: string;
  pvp_type: (typeof PVP_TYPES)[number];
  score: string | null;
  created_at: string;
  p1?: any;
  p2?: any;
  ...
  async function loadProfile(uname: string) {
    ...
    try {
      ...
      (challengeMatchesRes.data as any[] | null)?.forEach((m) => {
        ...
        events.push({ playerA: c.challenger, playerB: c.challenged, winner: m.winner, pvp_type: m.pvp_type as (typeof PVP_TYPES)[number], created_at: m.created_at });
      });
